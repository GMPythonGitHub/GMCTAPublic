...
> 初期化関数 \_\_init\_\_(xx, yy)， 文字列関数 \_\_str\_\_()  
...
> 円座標系における表示を可能にします  
>> インスタンス変数： \_\_xx, \_\_yy  
> 初期化関数 **\_\_init\_\_(xx, yy, rr, th, deg)**， 文字列関数 \_\_str\_\_()  
...
> インスタンス関数 leng(), dirc(), unitvect(), inner(), outer(), cross()  
> インスタンス関数 **conv()**, **add()**, **sub()**, **rsub()**, **mul()**, **div()**, **rdiv()**  
> インスタンス関数 **\_\_pos\_\_()**, **\_\_neg\_\_()**, 
...
ここで紹介する最初のクラスGMVecotrAでは，ベクトルの成分がインスタンス変数として所属し，
...
```
クラスの内外でグローバルに使える，クラス関数として２つの関数を定義しています。
...
```
クラスに所属するインスタンス変数は直交座標系における２成分で，前のクラスGMVectorAから変更はありません。
...
**［section_cd］** 以下のインスタンス関数に大きに変更はありませんが，ndarrayに適応した簡潔な記述にしています。
```python
    ## --- section_ce: functions for properties --- ##
    def leng(self) -> float:
        return self.rrth()[0]
...
**［section_cc］**  文字列関数に変更はありません。

**［section_cd］** ベクトルの特性を返すインスタンス関数に変更はありません。

**［section_ce］**  ベクトル解析の結果を返すインスタンス関数に変更はありません。
