# **Note on matrix equation: Type C; fixity condition is given**
...
> [gm_matrix_equation_c0_xx_list.py](gm_matrix_equation_c0_xx_list.py)  
> リスト構造（listとtuple）を用いて行列方程式を記述します。
> ベクトルの既知・未知条件に対応して部分行列方程式を誘導し，
> 「掃き出し法」により $X$ を求め，次いで $B$ を求めます。
> 
> [gm_matrix_equation_c1_xx_array.py](gm_matrix_equation_c1_xx_array.py)  
> 拡張モジュールnumpyが提供するndarrayを用いて行列方程式を記述します。
> 部分行列方程式を誘導して，「掃き出し法」により $X$ と $B$ を求めます。
> リスト構造を用いる場合と比較して，計算過程を簡潔にできます。
> 
> [gm_matrix_equation_c2_xx_array_func.py](gm_matrix_equation_c2_xx_array_func.py)  
